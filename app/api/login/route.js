...
/**
 * Generates the initial profits data for 12 months starting from the current month.
 * Each month will have 22 entries, all initialized to 0.
 * The profits object with month-year keys and arrays of zeros.
 */
function generateInitialProfitsData() {
  const profits = {};
  const date = new Date(); // Use current date to start
  const monthNames = [
  ...
  for (let i = 0; i < 12; i++) {
    ...
    profits[monthYearKey] = Array(22).fill(0);
...
export async function POST(request) {
  ...
  try {
    ...
    if (!userDocSnap.exists) {
      console.log(`Authorized new user: ${userEmail} (UID: ${uid}). Creating profile.`);
      const initialProfits = generateInitialProfitsData(); // Generate profits data
      ...
      await userDocRef.set({
        ...
        uid: uid,
        createdAt: FieldValue.serverTimestamp(),
        invtestedAmount: 0, // Corrected typo: 'invtestedAmount' to 'investedAmount' (assuming)
        activeSubscription: true,
        profits: initialProfits, // Add the generated profits object here
        // Add other default profile fields as needed
      });
      isNewUser = true;
    } else {
    ...
    const sessionCookie = await adminAuth.createSessionCookie(idToken, { expiresIn });
    const response = NextResponse.json({ status: 'success', isNewUser: isNewUser });
